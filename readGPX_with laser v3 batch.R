...
for (k in 1:length(all.files)){
  ...
  ggsave(p.altimeters, filename = paste0(fig.dir, filename.root, "_altimeters.png"),
         device = "png", dpi = 600)
  
  p.tracks <- ggplot(readings.df) + 
    geom_path(aes(x = lon, y = lat, color = ele)) + 
    geom_point(aes(x = lon[1], y = lat[1]), 
               color = "red", shape = "circle") +
    geom_point(aes(x = lon[nrow(readings.df)], y = lat[nrow(readings.df)]), 
               color = "blue", shape = "circle")
  
  ggsave(p.tracks, filename = paste0(fig.dir, filename.root, "_tracks.png"),
         device = "png", dpi = 600)
